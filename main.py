...
# -*- coding: utf-8 -*-
"""
PEMIF v22.4 ADAPTIVE INTELLIGENCE ENGINE — FULL BUG FIX + KZ FIX
════════════════════════════════════════════════════════════════
FIXES dari audit v22.3 (urutan kritis):

FIX-1 [KRITIS] MTF bias palsu karena snapshot cuma 60 bar M1
FIX-2 [KRITIS] Order Block bukan OB asli
FIX-3 [KRITIS] FVG tidak dicek mitigasi
FIX-4 [KRITIS] BOS dan liq_sweep "nempel" puluhan bar
FIX-5 [KRITIS] pd_priority tidak cek arah sinyal
FIX-6 [KRITIS] ML bias hardcoded "BUL"
FIX-7 [MEDIUM] Confidence% skalanya menipu

FIXES KILL ZONE (audit v22.4):
FIX-KZ-1 [KRITIS] Asian KZ aktif 12 jam/hari karena next_day_end=True
          tidak relevan setelah konversi ke WIB. Wrap-around
          sekarang dideteksi otomatis dari WIB times.
FIX-KZ-2 [KRITIS] check() pakai "is_next OR start_m > end_m" →
          wrap-around logic salah trigger untuk semua KZ.
FIX-KZ-3 [MEDIUM] current_kz_remaining() mewarisi bug KZ-1/2,
          return sisa menit salah.
FIX-KZ-4 [MEDIUM] next_killzone() tidak skip KZ aktif dan tidak
          handle wrap-around dengan benar.
FIX-KZ-5 [MEDIUM] get_all_windows_wib() display tampak benar
          tapi check() berperilaku berbeda — sekarang konsisten.
FIX-KZ-6 [MINOR]  _us_dst_active() off-by-one jika 1 Maret = Minggu
          (berlaku 2026). Rumus baru pakai first_sunday + 7 hari.
FIX-KZ-7 [MINOR]  RegimeAdaptiveMode.on_bar() streak terhitung
          seharian karena KZ selalu aktif → diperbaiki via KZ-1/2.

MINOR FIXES:
  - news filter tidak lagi hardcoded True/0
  - SL_SWING_BUFFER dikonversi ke ATR-scaled
  - Guard risk < 0.01 diganti ke risk < ATR * 0.05

FORMATTER UPGRADE (v22.4-FMT):
  - _bias_table_compact(): dua kolom, icon 🟢🟡🔴 per TF
  - _pd_array_block(): OB/FVG price level inline sesuai arah
  - _reason_to_enter(): narrative singkat "Reason to Enter"
  - _kz_stat_block(): winrate per KZ dari journal inline
  - fmt_signal_telegram(): semua section baru terintegrasi
  - fmt_scanning_telegram(): PD Array + bias compact + KZ stat
    (tanpa order level — hanya di signal penuh)
  - fmt_no_signal_telegram(): KZ stat + next KZ countdown
════════════════════════════════════════════════════════════════
"""
...
log = logging.getLogger("PEMIF-v22.4")
...
MIN_BARS_CME:      int = 60
MIN_BARS_STC:      int = 55
D1_BUFFER_BARS:    int = 1500
MTF_BARS_NEEDED:   int = D1_BUFFER_BARS
MAX_EVENT_GAP_BARS: int = 5
FVG_MITIGATION_THRESHOLD: float = 0.5
...
KILL_ZONES_NY: Tuple[Dict, ...] = (
    {"name": "Asian KZ",     "ny_start": (20, 0), "ny_end": (0,  0)},
    {"name": "London KZ",    "ny_start": (2,  0), "ny_end": (5,  0)},
    {"name": "NY Open KZ",   "ny_start": (7,  0), "ny_end": (10, 0)},
    {"name": "London Close", "ny_start": (10, 0), "ny_end": (12, 0)},
)
...
# ENVIRONMENT
# ═══════════════════════════════════════════════════════════════
def _load_env() -> Tuple[str, str, str, str, str, bool, int]:
    ...
    news_ok  = os.environ.get("PEMIF_NEWS_OK",  "true").strip().lower() in ("1", "true", "yes")
    news_tier = int(os.environ.get("PEMIF_NEWS_TIER", "0"))
    if not token or not chat_id:
        log.warning("Telegram credentials tidak di-set.")
    if not td_key:
        log.warning("TwelveData API key tidak di-set.")
    return token, chat_id, td_key, symbol, interval, news_ok, news_tier


TELEGRAM_TOKEN, TELEGRAM_CHATID, TWELVEDATA_KEY, SYMBOL, INTERVAL, \
    DEFAULT_NEWS_OK, DEFAULT_NEWS_TIER = _load_env()
...
def _us_dst_active(dt: datetime) -> bool:
    y = dt.year
    march1 = date(y, 3, 1)
    days_to_first_sun_mar = (6 - march1.weekday()) % 7
    first_sun_mar = date(y, 3, 1 + days_to_first_sun_mar)
    dst_start     = first_sun_mar + timedelta(weeks=1)
    nov1 = date(y, 11, 1)
    days_to_first_sun_nov = (6 - nov1.weekday()) % 7
    dst_end = date(y, 11, 1 + days_to_first_sun_nov)
...
# ═══════════════════════════════════════════════════════════════
# KILL ZONE SCHEDULER  — FIX-KZ-1 s/d FIX-KZ-5
# ═══════════════════════════════════════════════════════════════
class KillZoneScheduler:

    @staticmethod
    def _kz_wib_times(kz: Dict, offset: int) -> Tuple[int, int, int, int]:
        sh, sm = kz["ny_start"]
        eh, em = kz["ny_end"]
        return (sh + offset) % 24, sm, (eh + offset) % 24, em

    @staticmethod
    def _in_window(cur_min: int, start_m: int, end_m: int) -> bool:
        if start_m == end_m:
            return False
        if start_m > end_m:
            return (cur_min >= start_m) or (cur_min < end_m)
        return start_m <= cur_min < end_m

    @staticmethod
    def _remaining_minutes(cur_min: int, end_m: int, start_m: int) -> int:
        if start_m > end_m:
            if cur_min >= start_m:
    ...
    def check(self, now_wib: datetime) -> Tuple[bool, str, str, str]:
        ...
        for kz in KILL_ZONES_NY:
            wib_sh, sm, wib_eh, em = self._kz_wib_times(kz, offset)
            start_m = wib_sh * 60 + sm
            end_m   = wib_eh * 60 + em
            if self._in_window(cur_min, start_m, end_m):
                return (
                    True,
                    kz["name"],
                    f"{wib_sh:02d}:{sm:02d}",
    ...
    def get_all_windows_wib(self, now_wib: datetime) -> List[Dict]:
        offset   = _ny_to_wib_offset(now_wib)
        cur_min  = now_wib.hour * 60 + now_wib.minute
        in_kz, active_name, _, _ = self.check(now_wib)
        result   = []
        for kz in KILL_ZONES_NY:
            wib_sh, sm, wib_eh, em = self._kz_wib_times(kz, offset)
            start_m = wib_sh * 60 + sm
            end_m   = wib_eh * 60 + em
            is_wrap = start_m > end_m
            active  = in_kz and (kz["name"] == active_name)
            result.append({
                "name":   kz["name"],
    ...
    def current_kz_remaining(self, now_wib: datetime) -> int:
        ...
        for kz in KILL_ZONES_NY:
            ...
            end_m   = wib_eh * 60 + em
            if self._in_window(cur_min, start_m, end_m):
                return self._remaining_minutes(cur_min, end_m, start_m)
        return 0
        ...
        with self._lock:
            ...
            if can_switch and self.mode == "STRICT":
                if self._no_signal_streak_in_kz >= REGIME_NO_SIGNAL_BARS_TO_LOOSEN:
                    ...
                    switched_to = "LOOSE"
                    log.warning("REGIME SWITCH -> LOOSE (no signal %d bar dalam KZ)",
                                REGIME_NO_SIGNAL_BARS_TO_LOOSEN)

            elif can_switch and self.mode == "LOOSE":
                stats = self.journal.get_stats(symbol=self.symbol,
                                               last_n=REGIME_MIN_TRADES_FOR_WINRATE * 3)
                if (stats.total >= REGIME_MIN_TRADES_FOR_WINRATE and
                        stats.winrate < REGIME_WINRATE_MIN_TO_TIGHTEN):
                    self.mode = "STRICT"
                    self._bars_since_switch = 0
                    switched_to = "STRICT"
                    log.warning("REGIME SWITCH -> STRICT (winrate rolling %.1f%% < %.1f%%, n=%d)",
...
class Signal:
    ...
    bos_bull:        bool  = False
    bos_bear:        bool  = False
    bos_bull_age:    int   = 999
    bos_bear_age:    int   = 999
    ...
    liq_swept_l_age: int   = 999
    liq_swept_h_age: int   = 999
    disp_ok:         bool  = False
    sfp_signal:      str   = "NO"
...
class StructureEngine:
    ...
    def resample(bars_m1: List[Dict], bars_per_candle: int) -> List[Dict]:
        if bars_per_candle <= 1:
            return bars_m1
        if len(bars_m1) < bars_per_candle:
            return []
        out: List[Dict] = []
        n     = len(bars_m1)
    ...
    @staticmethod
    def detect_bos(
        bars: List[Dict], swing_high: float, swing_low: float, lookback_bars: int
        ...
        bos_bull_idx = bos_bear_idx = 999
        for i, b in enumerate(recent):
            if swing_high > 0 and b["close"] > swing_high and not bos_bull:
                bos_bull     = True
                bos_bull_idx = n - 1 - i
            if swing_low > 0 and b["close"] < swing_low and not bos_bear:
                bos_bear     = True
                bos_bear_idx = n - 1 - i
        return bos_bull, bos_bear, bos_bull_idx, bos_bear_idx
    ...
    def detect_liq_sweep(
        ...
        swept_h_age = swept_l_age = 999
        for i, b in enumerate(recent):
            age = n - 1 - i
            if swing_high > 0 and b["high"] > swing_high and b["close"] < swing_high:
                if not swept_h or age < swept_h_age:
                    swept_h     = True
                    swept_h_age = age
            if swing_low > 0 and b["low"] < swing_low and b["close"] > swing_low:
                if not swept_l or age < swept_l_age:
                    swept_l     = True
                    swept_l_age = age
        return swept_l, swept_h, swept_l_age, swept_h_age

    @staticmethod
    def detect_order_block(
        bars: List[Dict],
        bos_bull: bool, bos_bear: bool,
        bos_bull_idx: int, bos_bear_idx: int,
        max_age: int,
        ...
        if bos_bull and bos_bull_idx < n:
            bos_abs    = n - 1 - bos_bull_idx
            scan_start = max(0, bos_abs - max_age)
            for i in range(bos_abs - 1, scan_start - 1, -1):
                c = bars[i]
                if c["close"] < c["open"]:
                    ob_bull_high  = c["high"]
                    ob_bull_low   = c["low"]
        ...
        if bos_bear and bos_bear_idx < n:
            bos_abs    = n - 1 - bos_bear_idx
            scan_start = max(0, bos_abs - max_age)
            for i in range(bos_abs - 1, scan_start - 1, -1):
                c = bars[i]
                if c["close"] > c["open"]:
    ...
    @staticmethod
    def detect_fvg(
        bars: List[Dict], atr: float, min_gap_mult: float, max_age: int,
        mitigation_threshold: float = FVG_MITIGATION_THRESHOLD,
    ) -> Tuple[bool, bool, float, float, float, float]:
        n = len(bars)
        if n < 3 or atr <= 0:
            return False, False, 0.0, 0.0, 0.0, 0.0
            ...
            if not fvg_bull:
                ...
                if gap_up > min_gap:
                    ...
                    for j in range(i + 1, w):
                        fill = zh - window[j]["low"]
                        if fill / gap_size > mitigation_threshold:
                            mitigated = True
                            break
                    if not mitigated:
                        fvg_bull      = True
    ...
    def calc_bias(bars_tf: List[Dict], ema_fast: int, ema_slow: int) -> str:
        if not bars_tf or len(bars_tf) < max(MTF_MIN_CANDLES_FOR_BIAS, 2):
            return "NEU"
    ...
    def compute(
        ...
        swing_high, swing_low = self.find_swings(bars_cme, params["swing_lookback"])

        bos_bull, bos_bear, bos_bull_idx, bos_bear_idx = self.detect_bos(
            bars_cme, swing_high, swing_low, BOS_LOOKBACK_BARS)

        liq_swept_l, liq_swept_h, liq_swept_l_age, liq_swept_h_age = self.detect_liq_sweep(
            bars_cme, swing_high, swing_low, LIQ_SWEEP_LOOKBACK_BARS)

        (ob_bull_valid, ob_bear_valid, ob_bull_high, ob_bull_low,
         ob_bear_high, ob_bear_low) = self.detect_order_block(
            bars_cme, bos_bull, bos_bear,
            bos_bull_idx, bos_bear_idx,
            params["ob_max_age"])

        (fvg_bull_fresh, fvg_bear_fresh, fvg_bull_high, fvg_bull_low,
         fvg_bear_high, fvg_bear_low) = self.detect_fvg(
            bars_cme, atr, params["fvg_min_gap_mult"], params["fvg_max_age"])

        biases: Dict[str, str] = {}
        for tf_name, bars_per_candle in MTF_TIMEFRAMES.items():
            tf_bars        = self.resample(bars_full, bars_per_candle)
            biases[tf_name] = self.calc_bias(tf_bars, params["ema_fast"], params["ema_slow"])

        vol_surge = self.detect_vol_surge(bars_cme, params["vol_surge_mult"])
        acf_chop  = self.detect_chop(bars_cme, atr)
        disp_ok   = self.detect_displacement(bars_cme, atr)
        ...
        m1_bias = "NEU"
        if len(bars_cme) >= 2:
            m1_bias = "BUL" if bars_cme[-1]["close"] > bars_cme[-2]["close"] else "BER"

        return {
            "swing_high": round(swing_high, 2), "swing_low": round(swing_low, 2),
            "bos_bull": bos_bull, "bos_bear": bos_bear,
            "bos_bull_age": bos_bull_idx,
            "bos_bear_age": bos_bear_idx,
            "liq_swept_l": liq_swept_l, "liq_swept_h": liq_swept_h,
            "liq_swept_l_age": liq_swept_l_age,
            "liq_swept_h_age": liq_swept_h_age,
            ...
            "m5_bias":  biases.get("m5",  "NEU"), "m15_bias": biases.get("m15", "NEU"),
            "m30_bias": biases.get("m30", "NEU"), "h1_bias":  biases.get("h1",  "NEU"),
            "h4_bias":  biases.get("h4",  "NEU"), "d1_bias":  biases.get("d1",  "NEU"),
            "m1_bias": m1_bias,
            "vol_surge": vol_surge, "acf_chop": acf_chop, "disp_ok": disp_ok,
            "sfp_signal": sfp_signal, "pd_priority": pd_priority,
            "pd_type": "OB" if (ob_bull_valid or ob_bear_valid) else (
                "FVG" if (fvg_bull_fresh or fvg_bear_fresh) else "-"),
            "fractal_conv": sum([
                bos_bull or bos_bear,
                ob_bull_valid or ob_bear_valid,
                fvg_bull_fresh or fvg_bear_fresh,
                liq_swept_l or liq_swept_h,
            ]),
            "harmonic_pcz": False,
        }
...
class MLEngine:
    ...
    def _extract_features(self, t: Dict) -> Optional[List[float]]:
        try:
            ...
            target_bias = "BUL" if direction == "BUY" else "BER"
            ...
            return [
                ...
                1.0 if biases.get("d1")  == target_bias else 0.0,
                1.0 if biases.get("h4")  == target_bias else 0.0,
                1.0 if biases.get("h1")  == target_bias else 0.0,
                1.0 if biases.get("m30") == target_bias else 0.0,
                1.0 if biases.get("m15") == target_bias else 0.0,
                1.0 if biases.get("m5")  == target_bias else 0.0,
                1.0 if biases.get("m1")  == target_bias else 0.0,
            ]
        except Exception:
            return None
...
def calc_cms(bars: List[Dict], direction: str) -> Dict:
    ...
    hh = max(highs[-FISHER_PERIOD:])
    ll = min(lows[-FISHER_PERIOD:])
    fish = 0.0
    if hh != ll:
    ...
    return {
        "ttm_fire": ttm_fire, "lrsi_ok": lrsi_ok,
        "fisher_ok": fisher_ok, "stc_ok": stc_ok,
        "cms_score": round(min(pts, 10.0), 2), "atr": atr14,
    }
...
class PendingOrderEngine:
    ...
    def _bull(self, sig: Signal) -> OrderLevels:
        ...
        bos_sweep_confluent = (
            sig.bos_bull and sig.liq_swept_l and
            abs(sig.bos_bull_age - sig.liq_swept_l_age) <= MAX_EVENT_GAP_BARS
        )
        if sig.swing_high > 0 and bos_sweep_confluent:
            return self._stop("BUY", sig.swing_high, sig.swing_low,
                              f"BUY STOP breakout [{sig.swing_high:.2f}] (BOS+Sweep confluent)")
    ...
    def _bear(self, sig: Signal) -> OrderLevels:
        ...
        bos_sweep_confluent = (
            sig.bos_bear and sig.liq_swept_h and
            abs(sig.bos_bear_age - sig.liq_swept_h_age) <= MAX_EVENT_GAP_BARS
        )
        if sig.swing_low > 0 and bos_sweep_confluent:
            return self._stop("SELL", sig.swing_low, sig.swing_high,
                              f"SELL STOP breakdown [{sig.swing_low:.2f}] (BOS+Sweep confluent)")
        return OrderLevels(reason="Tidak ada zona SELL valid", valid=False)

    def _limit(self, d, zh, zl, sr, reason) -> OrderLevels:
        atr    = self.atr
        bull   = d == "BUY"
        sl_buf = atr * SL_SWING_ATR_MULT
        if bull:
            e  = max(zh - atr * OB_ENTRY_ATR_MULT, zl)
            sl = min(zl - sl_buf, e - atr * SL_ATR_MULT)
        else:
            e  = min(zl + atr * OB_ENTRY_ATR_MULT, zh)
            sl = max(zh + sl_buf, e + atr * SL_ATR_MULT)
    ...
    def _stop(self, d, level, sr, reason) -> OrderLevels:
        ...
        if bull:
            ...
            sl = min(sr - sl_buf, e - atr * SL_ATR_MULT)
        else:
            e  = level - atr * BREAKOUT_ATR_MULT
            sl = max(sr + sl_buf, e + atr * SL_ATR_MULT)
        return self._fin(f"{d} STOP", d, e, sl, reason)

    def _fin(self, ot, d, e, sl, reason) -> OrderLevels:
        risk     = abs(e - sl)
        min_risk = self.atr * 0.05
        if risk < min_risk:
            return OrderLevels(reason=f"Risk terlalu kecil ({risk:.4f} < {min_risk:.4f})", valid=False)
        s = 1 if d == "BUY" else -1
        return OrderLevels(
            order_type=ot, entry=round(e, 2), sl=round(sl, 2),
            tp1=round(e + s * risk * FIB_TP1, 2),
            tp2=round(e + s * risk * FIB_TP2, 2),
...
def calc_qcm(sig_data: Dict) -> int:
    ...
    has_directional_pd = (
        bool(sig_data.get("ob_bull_valid" if bull else "ob_bear_valid")) or
        bool(sig_data.get("fvg_bull_fresh" if bull else "fvg_bear_fresh"))
    )
    pd_prio_val = 5 if has_directional_pd else 15
    s += max(0, 15 - max(0, int(pd_prio_val)))
...
def check_gate(sig_data: Dict, params: AdaptiveParams) -> Tuple[bool, str]:
    ...
    if not sig_data.get("order_valid", False):
        return False, "ORDER LEVEL TIDAK VALID"
    if params.ml_active:
...
def analyze(
    ...
    atr_now = calc_atr_rma(bars_cme, ATR_PERIOD)

    struct = structure_engine.compute(bars_full, bars_cme, direction, atr_now, struct_params)
    ...
    cme = calc_cms(bars_cme, sig.direction)
    ...
    sig.atr_current = cme["atr"] or atr_now
...
def analyze_both_directions(
    ...
    price    = bars_cme[-1]["close"] if bars_cme else 0.0
    base_raw = {"current_price": price, "news_ok": news_ok, "news_tier": news_tier}

    sig_buy  = analyze({**base_raw, "direction": "BUY"},
                       bars_cme, bars_full, symbol, adaptive, regime)
    sig_sell = analyze({**base_raw, "direction": "SELL"},
                       bars_cme, bars_full, symbol, adaptive, regime)
...
# ═══════════════════════════════════════════════════════════════
# VISUAL HELPERS  — UPGRADE v22.4-FMT
# ═══════════════════════════════════════════════════════════════
...
def _conf(eps, sqs, ctx, soft) -> int:
    return round(
        min(eps / MAX_EPS, 1) * 30 +
        min(sqs / MAX_SQS, 1) * 30 +
        min(ctx / MAX_CTX, 1) * 20 +
        min(soft / MAX_SOFT_REAL, 1) * 20
    )
...
def _bias_table_compact(sig: Signal) -> str:
    """
    Bias table dua kolom dengan icon warna per TF.
    🟢 = aligned  🟡 = NEU  🔴 = berlawanan
    """
    d  = sig.direction
    ...
    rows = [
        ("D1 ", sig.d1_bias,  "H1 ", sig.h1_bias),
        ("H4 ", sig.h4_bias,  "M30", sig.m30_bias),
        ("M15", sig.m15_bias, "M5 ", sig.m5_bias),
        ("M1 ", sig.m1_bias,  "",    ""),
    ]
    lines = []
    for tf_l, v_l, tf_r, v_r in rows:
        left  = f"{ico(v_l)} {tf_l} <code>{v_l:<3}</code>"
        right = f"  {ico(v_r)} {tf_r} <code>{v_r:<3}</code>" if tf_r.strip() else ""
        lines.append(left + right)
    return "\n".join(lines)

def _pd_array_block(sig: Signal) -> str:
    """
    PD Array dengan price level inline sesuai arah sinyal.
    Tampil: OB, FVG, Swing H/L, Liq Sweep.
    """
    d    = sig.direction
    bull = d == "BUY"
    lines = []
    ...
    # Order Block
    if bull and sig.ob_bull_valid and sig.ob_bull_high > 0:
        lines.append(
            f"🟩 <b>OB Bull</b>  : <code>{sig.ob_bull_low:.2f} – {sig.ob_bull_high:.2f}</code>"
        )
    elif not bull and sig.ob_bear_valid and sig.ob_bear_high > 0:
        lines.append(
            f"🟥 <b>OB Bear</b>  : <code>{sig.ob_bear_low:.2f} – {sig.ob_bear_high:.2f}</code>"
        )
    else:
        lines.append("⬜ OB        : —")

    # FVG
    if bull and sig.fvg_bull_fresh and sig.fvg_bull_high > 0:
        lines.append(
            f"🟦 <b>FVG Bull</b> : <code>{sig.fvg_bull_low:.2f} – {sig.fvg_bull_high:.2f}</code>"
        )
    elif not bull and sig.fvg_bear_fresh and sig.fvg_bear_high > 0:
        lines.append(
            f"🟪 <b>FVG Bear</b> : <code>{sig.fvg_bear_low:.2f} – {sig.fvg_bear_high:.2f}</code>"
        )
    else:
        lines.append("⬜ FVG       : —")

    # Swing
    if sig.swing_high > 0 or sig.swing_low > 0:
        lines.append(
            f"📊 Swing H/L : <code>{sig.swing_high:.2f}</code> / <code>{sig.swing_low:.2f}</code>"
        )

    # Liquidity sweep
    if bull and sig.liq_swept_l:
        lines.append(f"💧 Liq Sweep : Low swept (age={sig.liq_swept_l_age}b) ✅")
    elif not bull and sig.liq_swept_h:
        lines.append(f"💧 Liq Sweep : High swept (age={sig.liq_swept_h_age}b) ✅")
    else:
        lines.append("💧 Liq Sweep : —")

    return "\n".join(lines)

def _reason_to_enter(sig: Signal) -> str:
    """
    Narrative singkat 'Reason to Enter' berdasarkan kondisi aktif.
    ...
    if htf_aligned >= 2:
        reasons.append(
            f"HTF aligned {htf_aligned}/3 "
            f"({'D1+H4+H1' if htf_aligned == 3 else 'D1+H4' if sig.d1_bias == tb and sig.h4_bias == tb else 'D1+H1' if sig.d1_bias == tb else 'H4+H1'}) "
            f"mendukung arah {d}."
        )

    # PD Array
    if bull:
        if sig.ob_bull_valid:
            reasons.append(f"OB Bull aktif [{sig.ob_bull_low:.2f}–{sig.ob_bull_high:.2f}] sebagai zona entry.")
        elif sig.fvg_bull_fresh:
            reasons.append(f"FVG Bull fresh [{sig.fvg_bull_low:.2f}–{sig.fvg_bull_high:.2f}] belum termitigasi.")
    else:
        if sig.ob_bear_valid:
            reasons.append(f"OB Bear aktif [{sig.ob_bear_low:.2f}–{sig.ob_bear_high:.2f}] sebagai zona entry.")
        elif sig.fvg_bear_fresh:
            reasons.append(f"FVG Bear fresh [{sig.fvg_bear_low:.2f}–{sig.fvg_bear_high:.2f}] belum termitigasi.")

    # Structure + Liquidity
    struct_parts = []
    if (sig.bos_bull if bull else sig.bos_bear):
        struct_parts.append("BOS konfirmasi")
    if (sig.liq_swept_l if bull else sig.liq_swept_h):
        struct_parts.append("liquidity swept")
    if sig.disp_ok:
        struct_parts.append("displacement valid")
    if sig.sfp_signal in ("BULL", "BEAR"):
        struct_parts.append("SFP terkonfirmasi")
    if struct_parts:
        reasons.append(f"Konfluensi: {', '.join(struct_parts)}.")

    # Fallback
    if not reasons:
        reasons.append(f"Sinyal {d} dengan EPS {sig.eps_score}/{MAX_EPS} dan QCM {sig.qcm_score}.")

    return " ".join(reasons[:3])

def _kz_stat_block(journal: "TradeJournal", symbol: str) -> str:
    """
    Winrate per Kill Zone dari journal — inline di pesan.
    Minimum 5 trades agar tampil, else tampil '-'.
    """
    MIN_T = 5
    kz_stats = journal.get_stats_by_kz(symbol=symbol, last_n=200)
    lines = []
    for kz_name in ["Asian KZ", "London KZ", "NY Open KZ", "London Close"]:
        st = kz_stats.get(kz_name, HistoricalStats())
        if st.total >= MIN_T:
            bar  = _bar(st.winrate, 100, 6)
            icon = "🟢" if st.winrate >= 55 else "🟡" if st.winrate >= 45 else "🔴"
            lines.append(
                f"{icon} {kz_name:<14}: [{bar}] {st.winrate:.0f}% ({st.total}T | AvgRR {st.avg_rr})"
            )
        else:
            lines.append(f"⚪ {kz_name:<14}: — (data < {MIN_T}T)")
    return "\n".join(lines)
...
# ═══════════════════════════════════════════════════════════════
# FORMATTERS  — UPGRADE v22.4-FMT
# ═══════════════════════════════════════════════════════════════
def fmt_signal_telegram(sig: Signal, symbol: str, stats: HistoricalStats,
                        journal: Optional["TradeJournal"] = None) -> str:
    """
    Signal penuh: semua section lengkap termasuk order level,
    PD Array detail, bias compact, reason to enter, KZ stat.
    """
    ...
    if journal is not None:
        kz_stat_str = (
            f"\n\n═══════════════════════════════════\n\n"
            f"📊 <b>KILL ZONE STATS</b> (last 200T)\n\n"
            f"{_kz_stat_block(journal, symbol)}"
        )
    ...
    return (
        ...
        f"💡 <b>REASON TO ENTER</b>\n\n"
        f"<i>{_reason_to_enter(sig)}</i>\n\n"
        ...
        f"📐 <b>MTF ALIGNMENT</b>  ({aligned}/7 aligned)\n\n"
        f"{_bias_table_compact(sig)}\n\n"
        ...
        f"SOFT       : {sig.soft_count}/{MAX_SOFT_REAL}\n"
        ...
        f"{SEP}\n"
        f"🕒 {now_wib}\n"
        f"<i>PEMIF v22.4 Adaptive Intelligence</i>\n"
        f"{SEP}"
    )


def fmt_scanning_telegram(sig: Signal, symbol: str,
                          journal: Optional["TradeJournal"] = None) -> str:
    """
    Scanning message: tanpa order level (hanya di signal penuh).
    Ditambah: PD Array detail, bias compact, reason to enter, KZ stat inline.
    """
    ...
    SEP = "─" * 33
    return (
        f"⏳ <b>SCANNING</b> | {symbol} | {now_wib} | {_dir_icon(sig.direction)} {sig.direction}\n\n"
...
def fmt_no_signal_telegram(sig: Signal, symbol: str,
                           journal: Optional["TradeJournal"] = None) -> str:
    """
    OFF Kill Zone message: ringkas dengan next KZ countdown + KZ stat.
    """
    now_wib = datetime.now(WIB).strftime("%H:%M WIB")
    ...
    return (
        f"🤖 <b>PEMIF v22.4</b> | {symbol} | {now_wib}\n"
        f"Status : OFF Kill Zone ({dst_tag}) | Regime: {sig.regime_mode}\n\n"
        f"─────────────────────────────────\n\n"

        f"📊 Score Terakhir ({_dir_icon(sig.direction)} {sig.direction})\n\n"
...
def fmt_kz_schedule(now_wib: datetime) -> str:
    ...
    for w in windows:
        active = " ← AKTIF" if w.get("active") else ""
        wrap   = " [wrap]"  if w.get("wrap")   else ""
        lines.append(f"  🕐 {w['name']}: {w['start']} – {w['end']}{wrap}{active}")
    return "\n".join(lines)
...
class PriceStream:
    ...
    def get_bars_full(self) -> List[Dict]:
        with self._lock:
    ...
    def _ws_msg(self, ws, message):
        try:
            ...
            if p > 0:
                self._tick(p, float(d.get("timestamp", time.time())))
        except:
            pass
    ...
    def _run_rest(self):
        while not self._stop_evt.is_set():
            try:
                ...
                if vals:
                    self._append(self._pbar(vals[0]))
            except Exception as e:
                log.warning("REST: %s", e)
    ...
    def _fetch_initial(self):
        ...
        try:
            ...
            log.info("Initial: %d bars dimuat (target D1_BUFFER=%d).",
                     len(self._bars), D1_BUFFER_BARS)
        except Exception as e:
            log.error("Fetch initial: %s", e)
    ...
    def _tick(self, price: float, ts: float):
        ...
        with self._lock:
            if self._current_bar is None:
                ...
                if len(self._bars) > D1_BUFFER_BARS * 2:
                    self._bars = self._bars[-D1_BUFFER_BARS:]
                self._current_bar = {
                    "open": price, "high": price, "low": price,
                    "close": price, "volume": 1.0, "bar_ts": bts,
                    "datetime": datetime.fromtimestamp(bts, tz=WIB).isoformat(),
                ...
                if self.on_bar_close and len(self._bars) >= MIN_BARS_CME:
                    bars_cme  = list(self._bars[-MIN_BARS_CME:])
                    bars_full = list(self._bars)
                    threading.Thread(
                        target=self.on_bar_close, args=(bars_cme, bars_full), daemon=True
                    ).start()
            else:
...
# ═══════════════════════════════════════════════════════════════
# MAIN ENGINE  — journal diteruskan ke formatter
# ═══════════════════════════════════════════════════════════════
def run_engine(
    symbol:   str = SYMBOL,
    ...
    log.info("PEMIF v22.4 starting: %s @ %s | Regime awal=%s", symbol, interval, regime.get_mode())
    send_telegram(fmt_kz_schedule(datetime.now(WIB)))
    send_telegram(f"🚀 PEMIF v22.4 aktif. Regime mode awal: <b>{regime.get_mode()}</b>")

    _last_kz_hour  = {"h": -1}
    _scan_counter  = {"n": 0}
    SCAN_MSG_EVERY = 5

    def on_bar_close(bars_cme: List[Dict], bars_full: List[Dict]) -> None:
        try:
            now   = datetime.now(WIB)
            stats = journal.get_stats(symbol=symbol, last_n=100)

            sig = analyze_both_directions(
                bars_cme=bars_cme, bars_full=bars_full,
                symbol=symbol, adaptive=adaptive, regime=regime,
                news_ok=DEFAULT_NEWS_OK, news_tier=DEFAULT_NEWS_TIER,
            )
            ...
            if sig.gate_ok and sig.order.valid:
                # ── Signal penuh: sertakan journal untuk KZ stat ──
                msg = fmt_signal_telegram(sig, symbol, stats, journal=journal)
                ...
                if _scan_counter["n"] >= SCAN_MSG_EVERY:
                    # ── Scanning: sertakan journal untuk KZ stat ──
                    msg = fmt_scanning_telegram(sig, symbol, journal=journal)
                ...
                if _scan_counter["n"] % 30 == 0:
                    # ── OFF-KZ: sertakan journal untuk KZ stat ──
                    send_telegram(fmt_no_signal_telegram(sig, symbol, journal=journal))
                _scan_counter["n"] += 1
...
def _plot_validation(bars: List[Dict], struct: Dict, atr: float,
    ...
    mpf.plot(df, type="candle", ax=ax_main, style="charles",
    ...
    for level, color, style, label in zip(
        ...
        ax_main.text(len(df) - 1, level, f"  {label}",
    ...
    for rect in markers["rects"]:
        ...
        ax_main.text(2, mid_y, rect["label"], color=rect["color"], fontsize=8,
                     va="center", fontweight="bold",
                     bbox=dict(facecolor="white", alpha=0.6, edgecolor="none", pad=1))

    last_idx = len(df) - 1
    if struct["bos_bull"]:
        ax_main.annotate("BOS BULL ▲", xy=(last_idx, df["High"].iloc[-1]),
    ...
    if struct["bos_bear"]:
        ax_main.annotate("BOS BEAR ▼", xy=(last_idx, df["Low"].iloc[-1]),
    ...
    if not fvg_text:
        fvg_text.append("FVG: tidak ada gap fresh (atau sudah ter-mitigasi)")

    info_lines = [
        f"Symbol: {symbol}  |  Interval: {interval}  |  Regime: {regime_mode}  [v22.4]",
        f"ATR({ATR_PERIOD}): {atr:.4f}  |  SL_SWING=ATR×{SL_SWING_ATR_MULT}  |  FVG_MITIG≥{FVG_MITIGATION_THRESHOLD:.0%}",
        f"Swing High: {struct['swing_high']:.2f}   Swing Low: {struct['swing_low']:.2f}",
        f"BOS Bull: {struct['bos_bull']} (age={struct['bos_bull_age']}b)   "
        f"BOS Bear: {struct['bos_bear']} (age={struct['bos_bear_age']}b)",
        f"OB Bull Valid: {struct['ob_bull_valid']}   OB Bear Valid: {struct['ob_bear_valid']}",
        f"Liq Swept L: {struct['liq_swept_l']} (age={struct['liq_swept_l_age']}b)   "
        f"Liq Swept H: {struct['liq_swept_h']} (age={struct['liq_swept_h_age']}b)",
        " | ".join(fvg_text),
    ...
    ax_main.set_title(
        f"PEMIF Structure Validator v22.4 — {symbol} ({interval}) — Mode: {regime_mode}",
...
def run_validator(
    ...
    outputsize = D1_BUFFER_BARS + 50
    print(f"📡 Mengambil {outputsize} bar terbaru dari TwelveData (D1 buffer)...")
    bars_full = _fetch_bars_rest(symbol=symbol, interval=interval,
                                 api_key=api_key, outputsize=outputsize)
    print(f"   Diterima: {len(bars_full)} bar | "
          f"Range: {bars_full[0]['datetime']} -> {bars_full[-1]['datetime']}")

    if len(bars_full) < MIN_BARS_CME:
        print(f"❌ Bar tidak cukup ({len(bars_full)} < {MIN_BARS_CME}).")
        sys.exit(1)

    bars_cme = bars_full[-MIN_BARS_CME:]
    atr      = calc_atr_rma(bars_cme, ATR_PERIOD)
    params   = _resolve_struct_params(REGIME_INITIAL_MODE)
    struct   = structure_engine.compute(bars_full, bars_cme, "BUY", atr, params)

    print("\n── Hasil StructureEngine v22.4 ──")
    for k in ("swing_high", "swing_low",
              "bos_bull", "bos_bear", "bos_bull_age", "bos_bear_age",
              "ob_bull_valid", "ob_bear_valid",
              "fvg_bull_fresh", "fvg_bear_fresh",
              "fvg_bull_high", "fvg_bull_low", "fvg_bear_high", "fvg_bear_low",
              "liq_swept_l", "liq_swept_l_age", "liq_swept_h", "liq_swept_h_age",
              "d1_bias", "h4_bias", "h1_bias", "m30_bias", "m15_bias", "m5_bias", "m1_bias",
              "vol_surge", "acf_chop", "disp_ok"):
        print(f"   {k:22s}: {struct[k]}")

    ts        = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = Path(f"pemif_validation_{ts}.png")
    _plot_validation(bars=bars_cme, struct=struct, atr=atr,
                     symbol=symbol, interval=interval,
                     regime_mode=REGIME_INITIAL_MODE, save_path=save_path)


def _quick_diagnostic() -> None:
    log.info("=== QUICK DIAGNOSTIC v22.4 START ===")
    log.info("--- KZ SCHEDULER SELF-TEST ---")
    now_wib = datetime.now(WIB)
    in_kz, kz_name, kz_s, kz_e = kz_scheduler.check(now_wib)
    log.info("check()          : in_kz=%s name=%s %s-%s", in_kz, kz_name, kz_s, kz_e)
    remaining = kz_scheduler.current_kz_remaining(now_wib)
    log.info("remaining()      : %d menit", remaining)
    nxt = kz_scheduler.next_killzone(now_wib)
    log.info("next_killzone()  : %s", nxt)
    windows = kz_scheduler.get_all_windows_wib(now_wib)
    for w in windows:
        log.info("  window: %s %s-%s wrap=%s active=%s",
                 w["name"], w["start"], w["end"], w["wrap"], w["active"])
    log.info("--- KZ SELF-TEST END ---")

    outputsize = D1_BUFFER_BARS + 50
    bars_full  = _fetch_bars_rest(SYMBOL, INTERVAL, TWELVEDATA_KEY, outputsize)
    log.info("Bar diterima: %d (butuh min %d untuk CME, %d untuk D1 bias)",
             len(bars_full), MIN_BARS_CME, D1_BUFFER_BARS)
    if len(bars_full) < MIN_BARS_CME:
        log.error("❌ Bar < %d. Stop.", MIN_BARS_CME)
        return
    ...
    log.info("bos_bull/bear    : %s (age=%d) / %s (age=%d)",
             sig.bos_bull, sig.bos_bull_age, sig.bos_bear, sig.bos_bear_age)
    log.info("ob_bull/bear_vld : %s / %s", sig.ob_bull_valid, sig.ob_bear_valid)
    log.info("fvg_bull/bear    : %s / %s", sig.fvg_bull_fresh, sig.fvg_bear_fresh)
    log.info("liq_swept_l/h    : %s (age=%d) / %s (age=%d)",
             sig.liq_swept_l, sig.liq_swept_l_age,
             sig.liq_swept_h, sig.liq_swept_h_age)
    log.info("Bias MTF  d1=%s h4=%s h1=%s m30=%s m15=%s m5=%s m1=%s",
             sig.d1_bias, sig.h4_bias, sig.h1_bias,
             sig.m30_bias, sig.m15_bias, sig.m5_bias, sig.m1_bias)
    log.info("eps=%d qcm=%d cms=%.1f", sig.eps_score, sig.qcm_score, sig.cms_score)
    log.info("in_killzone=%s kz=%s", sig.in_killzone, sig.kz_name)
    log.info("gate_ok=%s veto=%s", sig.gate_ok, sig.veto_rsn)
    log.info("=== QUICK DIAGNOSTIC v22.4 END ===")
